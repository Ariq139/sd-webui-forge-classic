...
The MMGP attention backend applies to Forge's core attention dispatch; Automatic preserves Forge's normal priority, while explicit Sage, Sage2, Sage3, Flash, SDPA, or xformers selection is used only when that backend is available. Radial is reserved for model-provided sparse masks. VAE attention has its own backend selector.<br>
